...
class MYPDF extends TCPDF
{
    ...
    public function ColoredTable($header, $data)
    {
        ...
        foreach ($data as $k => $v) {
            $this->Ln();
            $this->Cell($w[0], 6, $v['id_monitore'], 'LR', 0, 'L', $fill);
            $this->Cell($w[1], 6, $v['server'], 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $v['date_event'], 'LR', 0, 'L', $fill);
            $status='';
            if($v['status']==1){
                $status ="OK";
            }else{
...
$pdf->SetTitle('REPORTES EVENTOS');

$header_logo='images/logo.jpg';
// set default header data
$pdf->SetHeaderData($header_logo, PDF_HEADER_LOGO_WIDTH, 'TELCEL', 'REPORTE DE EVENTOS');
...
// column titles
$header = array('Id', 'Servidor',  'Fecha de ejecución', 'Estatus');
